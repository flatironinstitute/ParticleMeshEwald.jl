...
# particle mesh kernel truncated ewald, for non-periodic systems
mutable struct PMTE{T, TP, TD, TR, TC}
    alpha::T
    L::NTuple{3, T}
    s::T
    N::Int
    ...
    function PMTE(alpha::T, L::NTuple{3, T}, s::T, N::Int) where T
        eps = exp(-s^2) / s^2
        r_c = s / alpha
        k_c = 2 * s * alpha
        ...
        pos = zeros(T, 3, N);
        celllist = InPlaceNeighborList(x=pos, cutoff=r_c, parallel=true)
